...
mysqli_select_db($conn, 'rabinkarpss');
//error_reporting(0);
...
function char2hash($string) {
                if (strlen($string) == 1) {
                        ...
                        for ($i = 0; $i < $length; $i++) {
                                $result += ord(substr($string, $i, 1)) * pow(97, $length-$i);
                        }
                        return $result;
                }
        }
...
//                $output   = $stemmer->stem($baca1);
                $keys=   $system_answer[$key] = $value["abstrak"]; 
...
//               $readngram =  $system_answer[$key] =getNgrams("$keys","$kgrams"); 
    ...
    $readngram = explode(',',$keys);  
                ...
                sort($readngram);
//                print_r(array_unique($readngram9));
//                   echo"<br/>";
//                echo"<br/>";
//                echo"<br/>";
//                echo"<br/>";
//                
//                print_r(array_unique($readngram));
//                echo"<br/>";
//                echo"<br/>";
//                echo"<br/>";
//                echo"<br/>";
...
$jtotalarray9=count($readngram9);  
                $x=((2*$totals)/($jtotalarray+$jtotalarray9)*100);

//                echo round($x,2)." %";

                   
                 $conn->query("insert into hasil_uji(id_asli,id_uji,abstraksi,hasil) values('$id_dokumen','$randomid','$form','$x')");
			}
...
<div class="row content">
   <div class="col-md-12">
    ...
    <tbody>
        ...
        $sa = $conn->query("SELECT abstrak_uji.judul,hasil_uji.hasil FROM abstrak_uji RIGHT JOIN hasil_uji ON abstrak_uji.id=hasil_uji.id_asli WHERE hasil_uji.id_uji='$randomid' ORDER BY hasil_uji.hasil DESC LIMIT 5");
        foreach ($sa as $value) 
			{
            ?>
      <tr>
